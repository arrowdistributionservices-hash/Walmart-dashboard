...
import { getReconDataForDateRange, getAvailableReconReportDates } from "../../../lib/walmartClient";
 ...
 * for a specific order ID within a date range, plus the list of available
 * settlement report dates, so we can compare exactly what Walmart's API
 * reports against what the Seller Center UI shows.
 */
export async function GET(request) {
  try {
    ...
    const availableDates = await getAvailableReconReportDates();
    ...
    return NextResponse.json({
      availableReportDates: availableDates,
