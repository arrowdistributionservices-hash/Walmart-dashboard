...
export async function GET(request) {
  try {
    ...
    const daily = [...allDays].sort().map((date) => {
      ...
      const fees = reconByDay[date]?.fees || 0;
      const walmartTotal = ordersRev + incentive;
      const netAfterFees = walmartTotal + fees; // fees are typically negative (a cost); can be net positive if reimbursements exceed fees for the period
    ...
    const orderLevel = [...allOrderIds].map((orderId) => {
      ...
      const fees = reconByOrder[orderId]?.fees || 0;
    ...
    const totals = daily.reduce(
      ...
      { ordersRev: 0, incentive: 0, fees: 0, walmartTotal: 0, netAfterFees: 0, sellerboardRev: 0, itemCost: 0, profit: 0 }
    );
