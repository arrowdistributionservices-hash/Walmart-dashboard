...
import { getLatestSellerboardCsv, getLatestCostSheetCsv } from "../../../lib/storage";
import { parseSellerboardCsvText } from "../../../lib/sellerboardCsv";
import { parseCostSheetCsvText, lookupCost } from "../../../lib/costSheetCsv";
...
export async function GET(request) {
  try {
    ...
    // --- Cost sheet data (item cost / COGS, from last uploaded sheet, if any) ---
    const storedCostSheet = await getLatestCostSheetCsv();
    let costByKey = {};
    let costMeta = null;

    if (storedCostSheet?.csvText) {
      const parsed = parseCostSheetCsvText(storedCostSheet.csvText);
      costByKey = parsed.costByKey;
      costMeta = { filename: storedCostSheet.filename, uploadedAt: storedCostSheet.uploadedAt };
    }
