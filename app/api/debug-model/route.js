...
export async function GET(request) {
  try {
    ...
    const rawOrderId = searchParams.get("rawOrderId");
    let rawRowsForOrder = [];
    if (rawOrderId) {
      rawRowsForOrder = reconRows.filter(
        (r) =>
          String(r["Purchase Order #"] || "").trim() === rawOrderId ||
          String(r["Customer Order #"] || "").trim() === rawOrderId
      );
    }
