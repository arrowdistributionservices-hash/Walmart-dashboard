  ...
  return (
    <div className="page">
          ...
          {data?.settledThroughDate !== undefined && (
            <p className="meta-note" style={{ marginTop: 4 }}>
              ...
              {data?.settledThroughDate
                ...
                after that and still show provisional numbers (fees not yet deducted).`
                : " Rows/orders marked with * have fee data that isn't published yet - profit shown for those is provisional."}
            </p>
          )}
        </div>
            ...
            {data?.daily?.map((d) => (
              <tr key={d.date} className={d.feesPending ? "pending-row" : ""}>
                ...
                <td className={d.fees < 0 ? "bad" : ""}>
                  {fmt(d.fees)}
                  {d.feesPending && <span title="Not yet settled by Walmart">*</span>}
                </td>
                <td>{fmt(d.netAfterFees)}</td>
                <td>{fmt(d.itemCost)}</td>
                <td className={d.profit < 0 ? "bad" : ""}>
                  {fmt(d.profit)}
                  {d.feesPending && <span title="Provisional - fees not yet settled">*</span>}
                </td>
              </tr>
            ))}
            {!data?.daily?.length && (
            ...
            {data?.daily?.some((d) => d.feesPending) && (
              <tr>
                <td colSpan={8} style={{ textAlign: "center", color: "var(--text-dim)", fontSize: "0.85em" }}>
                  * Fees not yet published by Walmart for this day - profit shown is provisional and will drop once
                  settled.
                </td>
              </tr>
            )}
      ...
      <div className="controls" style={{ marginBottom: 10 }}>
        <input
          type="text"
          placeholder="Search by Order ID or amount (e.g. 45.20)"
          value={orderSearch}
            ...
            {filteredOrderLevel?.slice(0, 300).map((o) => (
              <tr key={o.orderId} className={o.feesPending ? "pending-row" : ""}>
                ...
                <td className={o.fees < 0 ? "bad" : ""}>
                  {fmt(o.fees)}
                  {o.feesPending && <span title="Not yet settled by Walmart">*</span>}
                </td>
                ...
                <td className={o.profit < 0 ? "bad" : ""}>
                  {fmt(o.profit)}
                  {o.feesPending && <span title="Provisional - fees not yet settled">*</span>}
                </td>
              </tr>
            ))}
