...
export default function Dashboard() {
  ...
  const filteredOrderLevel = !orderSearchNormalized
    ...
    : data?.orderLevel?.filter((o) => {
        ...
        if (o.itemSku && o.itemSku.toLowerCase().includes(orderSearchNormalized)) return true;
  ...
  return (
    <div className="page">
      ...
      {data?.feeCostBreakdown && (
        <div style={{ marginTop: 16 }}>
          <div className="section-title">
            Fee &amp; Cost Breakdown
            <span className="meta-note">from Walmart's settled transactions - matches Sellerboard's categories</span>
          </div>
          <div
            style={{
          ...
          >
            {[
              { key: "shippingCosts", title: "Shipping costs", expanded: shippingExpanded, setExpanded: setShippingExpanded },
              { key: "fees", title: "Fees", expanded: feesExpanded, setExpanded: setFeesExpanded },
            ].map(({ key, title, expanded, setExpanded }) => {
              const section = data.feeCostBreakdown[key];
              ...
              return (
                <div key={key} style={{ borderBottom: "1px solid var(--border)" }}>
                  ...
                  >
                    <span>
                      <span style={{ display: "inline-block", width: 14, color: "var(--text-dim)" }}>
                        {expanded ? "\u25BE" : "\u25B8"}
                      </span>
                      {title}
                    </span>
                    <span className={section.total < 0 ? "bad" : "good"}>{fmt(section.total)}</span>
                  </div>
                  {expanded &&
                    section.items.map((item) => (
                      ...
                      >
                        ...
                        <span className={item.total < 0 ? "bad" : "good"}>{fmt(item.total)}</span>
                      </div>
                    ))}
                  {expanded && !section.items.length && (
                    <div style={{ padding: "6px 14px 10px 34px", fontSize: 13, color: "var(--text-dim)" }}>
      ...
      <div className="controls" style={{ marginBottom: 10 }}>
        <input
          type="text"
          placeholder="Search by Order ID, item name, SKU, or amount (e.g. 45.20)"
          value={orderSearch}
            ...
            {filteredOrderLevel?.slice(0, 300).map((o) => (
              <tr key={o.orderId} className={o.feesPending ? "pending-row" : ""}>
                ...
                <td
                  ...
                  title={o.items?.map((it) => it.sku || "(no SKU)").join("\n") || ""}
                >
                  {o.itemSku || <span className="meta-note">-</span>}
                </td>
                <td>{o.date || "-"}</td>
