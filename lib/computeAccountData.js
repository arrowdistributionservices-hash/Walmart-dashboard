import {
  ...
  aggregateUnattributedFeesByDay,
...
export async function computeAccountData(accountId, { startDate, endDate }) {
  ...
  // Blanket account-level charges/reimbursements (no Purchase Order # or
  // Customer Order # - see aggregateUnattributedFeesByDay) can't be attached
  // to any order, so they're added directly into the day they posted rather
  // than through the per-order rollup below.
  const unattributedFeesByDay = aggregateUnattributedFeesByDay(settledReconRows);
  ...
  const allDays = new Set([
    ...
    ...Object.keys(unattributedFeesByDay),
  ...
  const daily = [...allDays].sort().map((date) => {
    ...
    const fees = (feesByDay[date] || 0) + (unattributedFeesByDay[date] || 0);
